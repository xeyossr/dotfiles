...
socket.on('load_messages_response', function(messages) {
    const messagesHTML = messages.map(message_data => {
        return `<div>
                    <strong><a href="#" onclick="addMention('${message_data.author_id}')">${message_data.author}</a></strong>: ${message_data.content}
                </div>`;
    }).join('');
...
// Python'dan mesajları almak
socket.on('messages_response', function(messages) {
    messages.forEach(function(message_data) {
        addMessageToChat(message_data);
    });
});

// Mesajları sohbet ekranına eklemek
function addMessageToChat(message_data) {
    const messageElement = document.createElement('div');
