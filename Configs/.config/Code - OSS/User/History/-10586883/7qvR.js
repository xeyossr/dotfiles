    ...
    menuButton.addEventListener('click', function() {
        ...
        server.channels.forEach(channel => {
            ...
            channelDiv.addEventListener('click', function() {
                selectChannel(server.id, channel.id);  // Kanal seçildiğinde fonksiyonu çağır
            });
...
// Sunucu ve kanal seçimlerini işleyen fonksiyonlar
function selectServer(serverId) {
    TARGET_SERVER_ID = serverId;
    // Seçilen sunucunun kanallarını yükle
    loadChannels(serverId);
}

function selectChannel(channelId) {
    TARGET_CHANNEL_ID = channelId;
    // Kanal seçildiğinde mesajları yüklemeye başla ve Python'a bu bilgileri gönder
    socket.emit('load_messages', { server_id: TARGET_SERVER_ID, channel_id: TARGET_CHANNEL_ID });
}
...
// Python'dan mesajlar geldiğinde bu fonksiyon tetiklenir
socket.on('load_messages_response', function(messages) {
    // Gelen mesajları mesajlar kısmına ekle
    messages.forEach(function(message_data) {
        addMessageToChat(message_data);
    });
});

// Eski mesaj ekleme fonksiyonu, yeniden kullanıyoruz
function addMessageToChat(message_data) {
    const messageElement = document.createElement('div');
    messageElement.innerHTML = `<strong>${message_data.author}</strong>: ${message_data.content}`;
    document.getElementById('messages').appendChild(messageElement);
