...
// Kanal seçildiğinde kanal ID'sini işlememiz gereken fonksiyon
function selectChannel(serverId, channelId) {
    console.log(`Seçilen sunucu ID'si: ${serverId}, kanal ID'si: ${channelId}`);
    // Bu noktada Python backend ile iletişime geçerek sunucu ve kanal bilgilerini işleyebilirsin
    fetch('/set_channel', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
    ...
    .then(response => response.json())
    .then(data => {
        console.log('Kanal seçildi!', data);
    })
    .catch(error => {
        console.error('Kanal seçilemedi:', error);
    });
}