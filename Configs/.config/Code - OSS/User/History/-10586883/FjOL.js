const socket = io({
        pingTimeout: 60000, // Timeout süresini 60 saniyeye ayarlayın
        pingInterval: 25000 // 25 saniyede bir ping gönderin
});
    ...
    menuButton.addEventListener('click', function() {
        ...
        server.channels.forEach(channel => {
            ...
            channelDiv.addEventListener('click', function() {
                ...
                selectServerChannel(Number(server.id),Number(channel.id));  // Kanal seçildiğinde fonksiyonu çağır
            });
...
function selectServerChannel(serverId, channelId) {
    ...
    socket.emit('load_messages_sync', {
        'server_id': TARGET_SERVER_ID,
        'channel_id': TARGET_CHANNEL_ID
    });
    console.log("load_messages_sync event emitted");
...
socket.on('load_messages_response', function(messages) {
    ...
    // Her 10 mesajda bir DOM'a ekleyelim
    if (messagesToDisplay.length >= 10) {
        addMessagesToChat(messagesToDisplay.splice(0, 10));
    }
});

function addMessagesToChat(messages) {
    const fragment = document.createDocumentFragment();  // Performansı artırmak için fragment kullanıyoruz
...
function addMessagesToChatAsync(messages) {
    let index = 0;
    ...
    function processNextMessage() {
        if (index < messages.length) {
            ...
            index++;
            requestAnimationFrame(processNextMessage);  // Bir sonraki animasyon karesinde mesajı ekle
        }
    }
