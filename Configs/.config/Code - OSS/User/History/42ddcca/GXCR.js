...
// Calculate butonu için olay dinleyici
document.querySelector('.calculate').addEventListener('click', async () => {
    const fen = game.fen(); // Mevcut pozisyonu FEN formatında al
    const bestMove = await getBestMove(fen, userColor); // Flask'ten en iyi hamleyi al
    if (bestMove) {
        game.move(bestMove); // En iyi hamleyi yap
        board.position(game.fen()); // Tahtadaki pozisyonu güncelle
        recordMove(bestMove, moveCount); // Hamleyi kaydet
        moveCount++;
    }
});


// Flask backend ile iletişime geçip en iyi hamleyi alan fonksiyon
const getBestMove = async (fenPosition, playerColor) => {
    try {
        const response = await fetch('http://127.0.0.1:5000/get_best_move', {
            method: 'POST',
            headers: {
