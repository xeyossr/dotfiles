...
menu(){
  printf "[Hypr] Edit hyprland.conf\n"
  printf "[Hypr] Edit windowrules.conf\n"
  printf "[Hypr] Edit keybindings.conf\n"
  printf "[Hypr] Edit animations.conf\n"
  printf "[Hypr] Edit themes/colors.conf\n"
  printf "[Hypr] Edit themes/common.conf\n"
  printf "[Hypr] Edit themes/theme.conf\n"
  printf "[Hypr] Edit pyprland.toml\n"
  printf "[Waybar] Edit config.ctl\n"
  printf "[Waybar] Edit style.css\n"
}
...
main() {
    ...
    case $choice in
        ...
        2)
            ...
            ;;
        3)
            $tty $editor "$HyprConfig/autostart.conf"
            ;;
        4)
            $tty $editor "$HyprConfig/binds.conf"
            ;;
        5)
            $tty $editor "$HyprConfig/settings.conf"
            ;;
        *)
            ;;
    esac
