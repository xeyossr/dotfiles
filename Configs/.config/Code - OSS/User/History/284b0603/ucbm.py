...
def check_tasks():
    ...
    for task in tasks:
        ...
        if task_when == "everyday" and current_hour_minute == task_time:
            if task["name"] not in last_notification_times or last_notification_times[task["time"]] != current_hour_minute:
                print(f"Her gün bildirimi: {task['description']}")
                send_notification(task['description'], task['image_url'])
                last_notification_times[task["name"]] = current_hour_minute
            else:
                print(f"Bu görev için zaten bildirim gönderildi: {task['name']}")
