...
#define IMAGE_VIEWER "w3mimgdisplay" // w3m aracını kullanarak terminalde görüntüleme
...
void print_random_pixel_art(const char *folder) {
    ...
    if (dp == NULL) {
        perror("Pixel art klasörü açılamadı.");
        return;
    }
    ...
    while ((entry = readdir(dp)) != NULL) {
        if (entry->d_type == DT_REG) { // Yalnızca dosyalar
            count++;
        }
    }

    if (count == 0) {
        printf("Pixel art bulunamadı.\n");
        closedir(dp);
        return;
    }
