...
def run_discord_bot(token):
                ...
                async for message in channel.history(limit=1000):
                    ...
                    if message.attachments:
                        if message.attachments[0].content_type.startswith('image/'):
                            ...
                            message_data['content'] = f'<video controls src="{message.attachments[0].url}" style="max-width: 250px;"></video>'

                    print(f"Eski mesaj: {message_data['content']} | Gönderen: {message_data['author']} | ID: {message_data['author_id']}")
    ...
    async def on_message(message):
        if message.guild.id == TARGET_SERVER_ID and message.channel.id == TARGET_CHANNEL_ID:
            ...
            if message.attachments:
                if message.attachments[0].content_type.startswith('image/'):
                    message_data['content'] = f'<img src="{message.attachments[0].url}" style="max-width: 250px;"/>'
                elif message.attachments[0].content_type.startswith('video/'):
                    message_data['content'] = f'<video controls src="{message.attachments[0].url}" style="max-width: 250px;"></video>'

            print(f"Yeni mesaj: {message.content} | Gönderen: {message.author.name}")  # .name ile erişim
            socketio.emit('new_message', message_data)
