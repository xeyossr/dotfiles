...
saved_messages = []  # Mesajları saklamak için liste
...
def run_discord_bot(token):
        ...
        if guild:
            ...
            if channel:
                ...
                async for message in channel.history(limit=1000):  # Son 1000 mesajı asenkron olarak çek
                    ...
                    # Eğer mesajda bir ek varsa, içeriği resim olarak güncelle
                    if message.attachments:
                        message_data['content'] = f'<img src="{message.attachments[0].url}" style="max-width: 200px;"/>'  # İlk ekin URL'sini al ve boyutlandır

                    saved_messages.append(message_data)  # Mesajı listeye ekle
                    socketio.emit('new_message', message_data)  # Eski mesajları soket üzerinden gönder
                print("Eski mesajlar yüklendi.")
            else:
    ...
    async def on_message(message):
        if message.guild.id == TARGET_SERVER_ID and message.channel.id == TARGET_CHANNEL_ID:
            ...
            # Eğer mesajda bir ek varsa ve içerik resim olarak başlamıyorsa, içeriği güncelle
            if message.attachments and not message_data['content'].startswith('<img'):
                message_data['content'] = f'<img src="{message.attachments[0].url}" style="max-width: 200px;"/>'  # İlk ekin URL'sini al ve boyutlandır

            print(f"Yeni mesaj: {message.content} | Gönderen: {message.author}")
            saved_messages.append(message_data)  # Yeni mesajı sakla
...
def get_saved_messages():
    return jsonify(saved_messages)  # Kaydedilmiş mesajları döndür
