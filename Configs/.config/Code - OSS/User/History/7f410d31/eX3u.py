    ...
    async def on_ready():
        ...
        if guild:
            ...
            if channel:
                messages = []
                # Eski mesajları çek
                async for message in channel.history(limit=300):  # Son 300 mesajı asenkron olarak çek
                    message_data = {
    ...
    if guild:
        ...
        if channel:
            ...
            async def fetch_messages():
                ...
                # Mesajları front-end'e geri gönderiyoruz
                socketio.emit('load_messages_response', messages)

            asyncio.run_coroutine_threadsafe(fetch_messages(), bot.loop)
