...
all_messages = {} # Tüm mesajları biriktir
    ...
    async def on_ready():
        ...
        for guild in bot.guilds:
            for channel in guild.text_channels:
                ...
                all_messages[guild.id] = all_messages.get(guild.id, {})
                all_messages[guild.id][channel.id] = messages
...
async def load_messages(data):
    ...
    print(f"Server ID: {TARGET_SERVER_ID}, Channel ID: {TARGET_CHANNEL_ID}")

    # Botun yüklü olduğundan emin olun
    if bot is None:
        print("Bot henüz başlatılmadı.")
        return
...
def load_messages_sync(data):
    ...
    messages = all_messages.get(server_id, {}).get(channel_id, [])
    socketio.emit('load_messages_response', messages)
