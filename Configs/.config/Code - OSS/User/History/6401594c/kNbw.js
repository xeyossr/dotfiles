...
module.exports = {
    ...
    async execute(interaction) {
        ...
        getWarnings(target.id, async (warnings) => {
            ...
            if (target.id == config.sahipid){
                interaction.reply({
                    content: '❌ Yüce Kamisaki-san uyarılamaz!',
                    ephemeral: true
                });
            }
            ...
            if (totalPoints >= 20) {
                try {
                    let member = await guild.members.fetch(target.id); // Üyeyi fetch et

                    if (member.bannable) { // Banlanabilir olup olmadığını kontrol et
                        const banReason = `Toplam 20 warn puanına ulaşıldı.`;
                        const banDuration = totalPoints; // x gün

                        // Veritabanına ban ekle
                        ...
                        embed.addField('Uyarı', '❌ Kullanıcı banlanamıyor. Yeterli yetki yok veya kullanıcıyı banlamak mümkün değil.');
                    }
                } catch (error) {
                    console.error('Banlama sırasında hata:', error);
                    return interaction.reply({ content: "❌ Kullanıcı banlanırken bir hata oluştu.", ephemeral: true });
                }
            }
