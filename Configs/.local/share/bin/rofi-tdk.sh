...
CACHE=~/.local/share/bin/rofi-tdk-cache
...
runrofi () {
    $(command -v rofi) -config ~/.config/rofi/clipboard.rasi -sorting-method fzf -sort true -normalize-match "$@"
}
...
while true; do
    ...
    case $(echo ${fl//[$'\t\r\n']} | head -n 1) in
        start) 
            word=$(cat $CACHE/list | runrofi -config ~/.config/rofi/clipboard.rasi -dmenu -p "TDK")
            echo -ne 'exit' > $TMP
            meaning "$word"
        ...
        ;;line) 
            ...
            line=$(cat $tmp | runrofi -dmenu -config ~/.config/rofi/clipboard.rasi $mesg "$sentence" -markup-rows -p 'TDK' -sep '|')
        ...
        ;;word) 
            ...
            selected=$(cat $tmp | rofi -p 'TDK' -config ~/.config/rofi/clipboard.rasi -dmenu -markup-rows -format "d:s")
